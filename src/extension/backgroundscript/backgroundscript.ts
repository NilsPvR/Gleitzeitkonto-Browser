...
async function sendMessageToCompanionApp(message) {
    try {
        ...
        portFromCS.postMessage({
            command: command,
            error: { message: errorMsgs.companionAppNotFound },
        });
    }
}
...
function connectedToContentScript(port) {
    ...
    portFromCS.onMessage.addListener((message) => {
        sendMessageToCompanionApp(message).then((response) => {
            // send command as is to companion app
            // check for any erros from the companionApp
            if (response?.error?.message) {
                console.error(
                    'Error in the Gleitzeitkonto-Browser CompanionApp: ' + response.error.message,
                );

                const command = message?.command.toLowerCase();
                portFromCS.postMessage({
                    command: command,
                    error: { message: errorMsgs.errorInCompanionApp },
                });
            } else if (response) {
