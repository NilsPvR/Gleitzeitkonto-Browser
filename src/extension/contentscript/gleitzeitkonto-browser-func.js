...
module.exports = class GleitzeitkontoBrowser {
    ...
    async updateInsertedDisplayOnChange(
        pHeaderBar,
        ...
        const placeOrRemoveInsertedDisplay = async () => {
            ...
            if (this.checkCorrectMenuIsOpen() && !this.getInsertedDisplay()) {
                const latestDisplayFormat = await this.getLatestDisplayFormat(
                    promiseCalcKontoData,
                    promiseDownloadKontoData,
                    promiseOutdatedIndex,
                );
                this.addInsertedDisplay(
                    pHeaderBar,
    ...
    reloadGleitzeitKonto() {
        ...
        promiseCalcKontoData.then(async () =>
            this.updateDisplay(
                await this.getLatestDisplayFormat(promiseCalcKontoData, promiseDownloadKontoData),
            ),
        );
        ...
        return new Promise((resolve, reject) => {
            ...
            const checkPageLoaded = setInterval(async () => {
                ...
                if (headerBar) {
                    ...
                    resolve(headerBar);
                } else if (loops > this.config.maxPageloadingLoops) {
    ...
    async getDownloadKontoData() {
        ...
        if (response == -1) {
            response = await this.sendMsgToBackgroundS(this.givenStrings.waitForDownlodCommand); // other request still downloading
        }

        if (response == 1) {
            kontoData.error = { message: this.constStrings.errorMsgs.incorrectPath, statusCode: 1 };
        } else if (response == 2) {
            kontoData.error = { message: this.constStrings.errorMsgs.notInNetwork, statusCode: 2 };
        } else if (response == 3) {
            kontoData.error = { message: this.constStrings.errorMsgs.tooManyCSV, statusCode: 3 };
        } else if (response == 4) {
            kontoData.error = { message: this.constStrings.errorMsgs.unknownAPI, statusCode: 4 };
        } else if (response == 0) {
            // success
    ...
    async checkVersionOutdated() {
        ...
        let localCompanionAppVersion = await this.sendMsgToBackgroundS(
            this.givenStrings.versionCommand,
        ); // get version obj
    ...
    formatDisplayText(kontoData) {
        if (kontoData?.error?.message)
            return this.constStrings.prefixError + kontoData.error.message; // Error occured
        if (!kontoData || !kontoData.kontoString)
            return (
                this.constStrings.prefixError +
                this.constStrings.errorMsgs.keineDatenVonCompanionApp
            );
        // No Data
    ...
    async getLatestDisplayFormat(
        ...
        if (this.globalFlags.versionCheckFinished && (await promiseOutdatedIndex) != 0) {
            // version outdated has highest priority
            if ((await promiseOutdatedIndex) == 1) {
                return {
                    text:
                        this.constStrings.prefixError +
                        this.constStrings.errorMsgs.extensionOutdated,
                    loading: false,
                };
                ...
                return {
                    text:
                        this.constStrings.prefixError +
                        this.constStrings.errorMsgs.companionAppOutdated,
                    loading: false,
                };
            }
        }
        if (this.globalFlags.downloadFinished) {
        ...
        // no data to show
        return {
            text: this.constStrings.prefixOvertime + this.constStrings.overtimeLoading,
            loading: true,
        };
    }
    ...
    addInsertedDisplay(pHeaderBar, pDisplayText, loading) {
        ...
        document.getElementById(this.constStrings.refreshIconID).addEventListener('click', () => {
            this.reloadGleitzeitKonto();
        });
    }
    ...
    getInnerHTMLElements(pDisplayText, loading, inserted) {
        ...
        const headline = this.createRichElement(
            ...
            pDisplayText ?? this.constStrings.errorMsgs.unknown,
        );
    ...
    startLoading() {
        const currentDisplay =
            document.getElementById(this.constStrings.insertedDisplayID) ??
    ...
    getLightingMode() {
        const header =
            document.getElementById(this.givenStrings.headerID) ??
