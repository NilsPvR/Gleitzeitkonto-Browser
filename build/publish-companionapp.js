...
(async function () {
    try {
        ...
        fs.copyFileSync(
            './src/companionapp/manifest.json',
            './compressed/companionapp/manifest.json',
        );
        fs.copyFileSync(
            './src/companionapp/manifest-chromium.json',
            './compressed/companionapp/manifest-chromium.json',
        );
        fs.copyFileSync(
            './src/companionapp/start-companionapp.bat',
            './compressed/companionapp/start-companionapp.bat',
        );
        // only copy the dependencies into the new package to avoid duplicate npm scripts for developers
        fs.writeFileSync(
            './compressed/companionapp/package.json',
            '{ "dependencies": ' +
